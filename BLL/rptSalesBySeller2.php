...
$result = $conn->query("SELECT S.dateStart, S.noDeliver, S.type, B.noDocument, B.date, B.amount, B.noReceipt, B.totalS, B.totalO
    FROM sale S INNER JOIN balance B ON S.idSale = B._idSale WHERE S.state = 0 AND B.balpay = 1 AND B.state = 0 AND
    B.date BETWEEN '$fi' AND '$ff' AND B._idSeller = $idSeller ORDER BY B.date ASC;");
$outp = array();
